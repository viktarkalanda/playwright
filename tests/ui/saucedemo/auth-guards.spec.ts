...
test.describe('Authentication guards and direct access', () => {
  test('anonymous cannot access inventory directly via URL', { tag: ['@auth', '@smoke'] }, async ({ page, loginPage }) => {
    await openInventoryDirect(page);
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('anonymous cannot access cart directly via URL', { tag: '@auth' }, async ({ page, loginPage }) => {
    await openCartDirect(page);
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('anonymous cannot access checkout step one directly', { tag: '@auth' }, async ({ page, loginPage }) => {
    await openCheckoutStepOneDirect(page);
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('anonymous cannot access checkout complete page directly', { tag: '@auth' }, async ({ page, loginPage }) => {
    await openCheckoutCompleteDirect(page);
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('anonymous cannot access product details page directly', { tag: '@auth' }, async ({ page, loginPage }) => {
    await openProductDetailsDirect(page);
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('user cannot open checkout step two without completing step one', { tag: ['@auth', '@checkout'] }, async ({ page, loginPage, checkoutStepOnePage }) => {
  ...
  test('user cannot open checkout complete page without finishing checkout', { tag: ['@auth', '@checkout'] }, async ({ page, loginPage, checkoutStepOnePage }) => {
    await loginPage.loginAs('standard');
    await openCheckoutCompleteDirect(page);
    await checkoutStepOnePage.waitForVisible();
    const title = await checkoutStepOnePage.getTitleText();
