// tests/ui/saucedemo/inventory.spec.ts
...
test.describe('Inventory', () => {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Product catalog — content and display
  // ──────────────────────────────────────────────────────────────────────────

  test.describe('Product catalog — content and display', () => {
    test(
    ...
    test(
      'inventory page shows all expected products',
      { tag: ['@inventory', '@smoke'] },
      async ({ inventoryPage }) => {
        const names = await inventoryPage.getItemNames();

        for (const expectedName of expectedProductNames) {
          expect(names, `Inventory page should contain product "${expectedName}"`).toContain(
            expectedName,
          );
        }
      },
    ...
    test(
      'inventory items count matches expected products length',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'inventory displays products in the default order',
      { tag: ['@inventory', '@smoke'] },
      async ({ inventoryPage }) => {
    ...
    test(
      'inventory exposes numeric prices for every product',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
        const prices = await inventoryPage.getAllItemPrices();

        expect(prices.length, 'Each listed product should have a price').toBe(
    ...
    test(
      'each inventory card shows name, price and action button',
      { tag: ['@inventory', '@smoke'] },
      async ({ inventoryPage }) => {
        ...
        for (let index = 0; index < itemCount; index += 1) {
          ...
          await expect(
            ...
            'Product name should be visible on card',
          ).toBeVisible();
          await expect(
            item.locator('.inventory_item_price'),
            'Product price should be visible on card',
          ).toBeVisible();
          await expect(
            item.getByRole('button'),
            'Action button should be visible on card',
          ).toBeVisible();
        }
      },
    );
  ...
  test.describe('Adding and removing items', () => {
    test(
      'cart is empty when user did not add any products',
      { tag: '@inventory' },
    ...
    test(
      'cart contains "Sauce Labs Backpack" after adding first product',
      { tag: '@inventory' },
      async ({ inventoryPage, cartPage }) => {
        await inventoryPage.addFirstItemToCart();
        await inventoryPage.openCart();
    ...
    test(
      'user can add two different products to cart',
      { tag: '@inventory' },
      async ({ inventoryPage, cartPage }) => {
        await inventoryPage.addFirstItemToCart();
        ...
        const itemsCount = await cartPage.getItemsCount();
        expect(itemsCount, 'Cart should contain exactly two items after adding two products').toBe(
          2,
        );
    ...
    test(
      ...
      async ({ inventoryPage, cartPage }) => {
        ...
        expect(
          itemsBeforeRemove,
          'Cart should contain at least one item before removal',
        ).toBeGreaterThan(0);
    ...
    test(
      'add to cart button changes to Remove after adding product by name',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
        ...
        expect(
          initialLabel?.trim(),
          'Initial button label should contain "Add to cart"',
    ...
    test(
      'remove button changes back to Add to cart after removing product by name',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
        const productName = 'Sauce Labs Backpack';
        await inventoryPage.addItemToCartByName(productName);
  ...
  test.describe('Cart badge', () => {
    test(
      'cart badge count is zero when no products are added',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'cart badge count increases when products are added from inventory',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'cart badge count decreases when product is removed from inventory',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
        await inventoryPage.addItemToCartByName('Sauce Labs Backpack');
        await inventoryPage.addItemToCartByName('Sauce Labs Bike Light');
    ...
    test(
      'cart badge matches actual cart items after multiple add/remove actions',
      { tag: '@inventory' },
      async ({ inventoryPage, cartPage }) => {
        ...
        expect(
          itemsInCart,
          'Cart should contain two items after removing one of the three added',
        ).toBe(2);
      },
    );

    test(
      'cart badge increments sequentially when adding multiple products',
      { tag: ['@inventory', '@smoke'] },
      async ({ inventoryPage }) => {
        const productsToAdd = [
          'Sauce Labs Backpack',
    ...
    test(
      'cart badge resets to zero after removing all items from inventory',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'cart badge matches cart page count after adding products on inventory',
      { tag: ['@inventory', '@e2e'] },
      async ({ inventoryPage, cartPage }) => {
        ...
        expect(cartItemsCount, 'Cart should list all products added from inventory').toBe(
          products.length,
        );
      },
    );
  ...
  test.describe('Sorting', () => {
    test(
      'inventory can sort products by name A to Z',
      { tag: ['@inventory', '@smoke'] },
      async ({ inventoryPage }) => {
    ...
    test(
      'inventory can sort products by name Z to A',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'inventory can sort products by price low to high',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
    ...
    test(
      'inventory can sort products by price high to low',
      { tag: '@inventory' },
      async ({ inventoryPage }) => {
  ...
  test.describe('Navigation and persistence', () => {
    test(
      'cart contents persist when navigating back to inventory and opening cart again',
      { tag: '@inventory' },
    ...
    test(
      'inventory reload does not remove products already added to the cart',
      { tag: '@inventory' },
      async ({ page, inventoryPage, cartPage }) => {
        await inventoryPage.addItemToCartByName('Sauce Labs Backpack');
    ...
    test(
      'sorting by name ascending survives navigating to cart and back',
      { tag: '@inventory' },
      async ({ inventoryPage, cartPage }) => {
        await inventoryPage.sortByNameAsc();
    ...
    test(
      ...
      async ({ inventoryPage, productDetailsPage }) => {
        ...
        expect(
          detailsPrice,
          'Product price on details page should match inventory list price',
        ).toBeCloseTo(listPrice, 2);
    ...
    test(
      'opening product details shows matching name and price',
      { tag: ['@inventory', '@e2e'] },
      async ({ inventoryPage, productDetailsPage }) => {
        ...
        expect(detailsName, 'Details page should show the selected product name').toBe(
          targetProduct,
        );
        expect(
          detailsPrice,
          'Details page should show the same price as inventory list',
